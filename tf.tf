...
# Elastic IPs for NAT Gateways (only if IGW is enabled and public subnets exist)
resource "aws_eip" "nat" {
  count = var.enable_nat_gateway && var.enable_internet_gateway && length(var.public_subnet_cidrs) > 0 ? (var.single_nat_gateway ? 1 : length(var.public_subnet_cidrs)) : 0
...
# NAT Gateways (only if IGW is enabled and public subnets exist)
