...
inputs:
  ...
  db_repository:
    description: >-
      trivy-db OCI ref. Include the :2 tag explicitly. Works with an ACR copy
      (myacr.azurecr.us/trivy/trivy-db:2) or a pull-through remote
      (myco.jfrog.io/ghcr-remote/aquasecurity/trivy-db:2).
      Leave blank only if runners can reach ghcr.io directly.
    required: false
    default: ""
  java_db_repository:
    description: "trivy-java-db OCI ref. Include the :1 tag explicitly."
    required: false
    default: ""
  db_anonymous:
    description: >-
      Set true when the DB mirror needs NO credentials (e.g. an anonymous JFrog
      pull-through). Blanks TRIVY_USERNAME/TRIVY_PASSWORD for the DB fetch so
      registry credentials are never sent to a third-party host.
    required: false
    default: "false"
  db_username:
    description: "Optional credentials for an authenticated non-ACR DB mirror."
    required: false
    default: ""
  db_password:
    required: false
...
runs:
  ...
  steps:
    ...
    # The database is fetched BEFORE any registry credential exists in the job
    # environment. TRIVY_USERNAME/TRIVY_PASSWORD are GLOBAL in Trivy - they are
    # sent to every registry it contacts, not only the one being scanned. If the
    # DB lives on a third-party mirror (JFrog, Harbor, a public proxy) and ACR
    # credentials are already exported, Trivy hands your ACR bearer token to
    # that third party. Do not reorder these two steps.
    - name: Refresh vulnerability database
      shell: bash
      env:
        DB_REPO: ${{ inputs.db_repository }}
        JAVA_DB_REPO: ${{ inputs.java_db_repository }}
        DB_ANONYMOUS: ${{ inputs.db_anonymous }}
        DB_USERNAME: ${{ inputs.db_username }}
        DB_PASSWORD: ${{ inputs.db_password }}
      run: |
        set -euo pipefail

        # Belt and braces: even if a future edit moves ACR auth above this step,
        # these explicit blanks override anything inherited from $GITHUB_ENV.
        if [[ "$DB_ANONYMOUS" == "true" ]]; then
          unset TRIVY_USERNAME TRIVY_PASSWORD TRIVY_REGISTRY_TOKEN || true
          export TRIVY_USERNAME="" TRIVY_PASSWORD=""
          echo "Fetching vulnerability DB anonymously (no credentials sent)."
        elif [[ -n "$DB_USERNAME" ]]; then
          export TRIVY_USERNAME="$DB_USERNAME"
          export TRIVY_PASSWORD="$DB_PASSWORD"
          echo "Fetching vulnerability DB with dedicated mirror credentials."
        else
          echo "Fetching vulnerability DB with ambient registry credentials."
        fi
    ...
    - name: Verify database freshness
      ...
      run: |
        ...
        if (( AGE > MAX_AGE )); then
          ...
          echo "::error::A stale DB silently passes images."
          echo "::error::Scheduled copy? Check the mirror job."
          echo "::error::Pull-through proxy? Check the upstream cache TTL - a proxy can"
          echo "::error::serve a cached manifest well past its NextUpdate."
          exit 1
        fi

    # Credentials appear only now, after the DB is already on disk.
    ...
    # --skip-db-update below is what keeps ACR credentials off the DB mirror:
    # the DB is already cached, so Trivy never contacts the mirror host again.
    - name: Scan image
      shell: bash
      env:
        IMAGE_REF: ${{ inputs.image_ref }}
