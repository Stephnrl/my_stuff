...
@description('Optional. Service endpoint object information for the Key Vault')
...
resource keyVault 'Microsoft.KeyVault/vaults@2023-02-01' = {
  name: name
  location: location
  tags: complianceTags.outputs.tags
  properties: {
    tenantId: tenantId
    sku: {
      family: 'A'
...
module keyVaultSecrets 'br/public:avm/res/key-vault/secret:1.0.0' = [for secret in secrets: if (!empty(secrets)) {
  ...
  params: {
    ...
    tags: contains(secret, 'tags') ? union(secret.tags, complianceTags.outputs.tags) : complianceTags.outputs.tags
  }
}]

// Create keys if specified
module keyVaultKeys 'br/public:avm/res/key-vault/key:1.0.0' = [for key in keys: if (!empty(keys)) {
  ...
  params: {
    ...
    tags: contains(key, 'tags') ? union(key.tags, complianceTags.outputs.tags) : complianceTags.outputs.tags
    rotationPolicy: contains(key, 'rotationPolicy') ? key.rotationPolicy : null
  }
}]

// Create private endpoints if specified
module keyVaultPrivateEndpoints 'private-endpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: if (!empty(privateEndpoints)) {
  ...
  params: {
    ...
    tags: contains(privateEndpoint, 'tags') ? union(privateEndpoint.tags, complianceTags.outputs.tags) : complianceTags.outputs.tags
  }
}]
...
output privateEndpointIds array = [for (privateEndpoint, index) in privateEndpoints: {
  privateEndpointId: keyVaultPrivateEndpoints[index].outputs.privateEndpointId
}]

@description('The compliance standard applied to this deployment')
